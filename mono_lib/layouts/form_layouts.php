...
class FormsLayout {
	...
	function formHeader($_link, $_id='') {
		if($_id == '')
			echo '<form action="'.$_link.'" method="post" enctype="multipart/form-data">';
		else
			echo '<form action="'.$_link.'" id="'.$_id.'" method="post" enctype="multipart/form-data">';
	}
	...
	function inputText($_name_on, $_size, $_value, $_name, $_id='', $_class='') {
		echo "<div class='$_class'>$_name_on<input type='text' size='$_size' value='$_value' name='$_name' id='$_id'/></div>";
	}
	...
	function inputCheckbox($_name_on, $_name, $_value=0, $_id='', $_is_checked=0) {
		if($_is_checked == 1)
			echo "<div><input type='checkbox' name='$_name' value='$_value' id='$_id' checked/>$_name_on</div>";
		else
			echo "<div><input type='checkbox' name='$_name' value='$_value' id='$_id'/>$_name_on</div>";
	}
